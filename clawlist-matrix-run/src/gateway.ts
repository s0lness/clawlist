...
export async function spawnGateway(
  ...
  if (requestedPort && (await portInUse(requestedPort))) {
    throw new Error(
      `Port ${requestedPort} is already in use. Stop the existing gateway or omit PORT to auto-pick.`
    );
  }
...
export async function stopGateway(gateway: GatewayProcess): Promise<void> {
...
export async function stopAllGateways(outDir: string): Promise<void> {
...
export async function stopGatewayByProfile(profile: string): Promise<void> {
...
export async function cleanupPorts(): Promise<void> {
  ...
  try {
    const portsInUse = await exec(
      `ss -ltnp 2>/dev/null | grep -E ':(1879[0-9]|188[0-9]{2})' | grep openclaw || true`
    );
